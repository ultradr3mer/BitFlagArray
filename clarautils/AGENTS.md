...
- **RankedBit**: Reihenfolge rank-first (erst alle Werte mit 1 Bit, dann 2, ...), innerhalb eines Rangs lexikographisch wie `itertools.combinations`; `bit_count` = Popcount der Maske (= mask_rank); empty (`bit_value=0`) hat `global_index` -1; `get_next(max_rank_idx)` grenzt Ranks ein (0-basierter Rang-Index), sonst Lauf bis volle Maske; `BitGroupWalker` verlangt paarweise disjunkte Masken, iteriert product-artig (innerste Gruppe zuerst) und liefert das OR als Combined-Zahl. `index_in_rank`/`_from_global_index`/`get_next` laufen über `RankIndexMin`/`BittyIndex`: Wertetuple → `row_from_cols` (key-Abstieg + Leaf-Daten), Zeile → `cols_from_row` (int-Abstieg + `key_path`), `get_next` = `_from_global_index(gidx+1)` mit Ceilings via `rank_states`; Tabellen je `(mask_rank, val_rank)` gecacht, Bitty wird nach `build` verworfen. Anker-Caches und `_lex_rank`/`_lex_unrank` sind entfernt.
- **BitFlagIndex**/`BittyIndex`: Index-Baum über `group_by_bit`. FluentBuilder: `index_by(key).then_by(*keys).with_leafs(key).index_by_key/slice/fullindex().build()`; Keys: `slice`/`int`/`list[int]` mit `group_by_bit`-Semantik (`list[int]` = EIN Level über mehrere Bit-Positionen; `then_by` ergibt ein Level pro Argument). `with_leafs(key)` fügt KEIN Level hinzu: die Gruppen der tiefsten `then_by`-Ebene werden direkt `LeafNode`s (`key_path` = Subdivisions-Werte, ohne Leaf-Wert); `key` selektiert `leaf.data` (`view.b[key]`) — der Leaf-Wert wird lokal über die Leaf-Key-Bits im Blatt aufgelöst. Voll keyed geht über `then_by`. Pro Level opt-in Maps: `key_index` (Default, wenn nichts explizit gesetzt), `slice_index` (Runs, `slice(a,b,1)`-Keys), `int_index` (absolute Root-Item-Indizes) — Maps zeigen immer auf das Kind des eigenen Levels. Blätter = `LeafNode(data, key_path, item_indices)` mit `data: np.ndarray`, materialisiert; `dispose_bitty=True` verwirft die Bitty nach `build()` + invalidiert deren Cache-Einträge.
...
- Index ist NICHT linear in den Positionen (LGS geprüft — polynomial mit Grad = Rang). Anker-Modell (auskommentiert, Nachfolge: direkte kombinatorische Rank/Unrank über `_lex_rank`/`_lex_unrank` bzw. `RankIndexMin`/`BittyIndex`): `_INDEX_CACHE` je Bit-Anzahl n, Umbruch-Anker (Positionen + Index), Lookup = Anker + Rest der letzten Spalte (`_gidx_of_positions`/`_positions_of_gidx`). `_lex_index`/`_lex_unrank`/`_next_positions` sind entfernt; die aktuellen `_lex_rank`/`_lex_unrank` sind neu und rechnen direkt.
