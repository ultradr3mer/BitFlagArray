...
def test_index_in_rank_matches_get_comb_idx(mask):
    for v in sub_values(mask):
        rb = RankedBit.from_value(v, mask)
...
def test_rank_info_floor(mask):
    n = mask.bit_count()
    for v in sub_values(mask):
        r_info = RankedBit.from_value(v, mask).get_rank_info()
...
def test_global_index_bijection(mask):
    ...
    gidx = [RankedBit.from_value(v, mask).global_index for v in seq]
...
def test_get_next_lex_order_4bit():
    seq = [b.bit_value for b in RankedBit.from_value(0, 0b1111).iter_next()]
...
def test_get_next_matches_bits_rank_first(mask):
    ...
    assert [b.bit_value for b in RankedBit.from_value(0, mask).iter_next()] == expected
...
def test_get_next_boundaries():
    full = RankedBit.from_value(0b1111, 0b1111)
    with pytest.raises(StopIteration):
        full.get_next()
    first = RankedBit.from_value(1, 0b1111)
    assert first.get_next().bit_value == 2


def test_get_next_max_rank_idx():
    mask = 0b1111
    seq1 = [b.bit_value for b in RankedBit.from_value(0, mask).iter_next(max_rank_idx=0)]
    assert seq1 == [1, 2, 4, 8]
    seq2 = [b.bit_value for b in RankedBit.from_value(0, mask).iter_next(max_rank_idx=1)]
    assert seq2 == [1, 2, 4, 8, 3, 5, 9, 6, 10, 12]
    with pytest.raises(StopIteration):
        RankedBit.from_value(0b1100, mask).get_next(max_rank_idx=1)
    with pytest.raises(StopIteration):
        RankedBit.from_value(0b1111, mask).get_next(max_rank_idx=1)


def test_add_sub():
    empty = RankedBit.from_value(0, 0b1111)
    ...
    with pytest.raises(IndexError):
        empty - 1
    with pytest.raises(IndexError):
        RankedBit.from_value(0b1111, 0b1111) + 1
...
def test_reprs():
    ...
    gapped = RankedBit.from_value(0b1001, 0b1011)
...
def test_walker_accepts_rankedbit_and_list():
    ...
    rb = RankedBit.from_value(0b10, 0b11)
